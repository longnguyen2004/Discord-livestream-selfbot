...
export class Bot extends EventEmitter {
    ...
    constructor({
        token, prefix, allowedId, modulesPath
    }: BotSettings)
    {
        super();
        ...
        (async () => {
            const modulesFile = (await glob("*/**/index.js", {
                cwd: modulesPath,
                dotRelative: true
            })).map(file => new URL(file, modulesPath + '/').toString());
            const modules = await Promise.all(
                modulesFile.map(file => import(file).then(m => m.default as Module))
            );
    ...
    private async _handleMessage(message: Message) {
        ...
        if (!this._allowedId.has(message.author.id))
            return;
